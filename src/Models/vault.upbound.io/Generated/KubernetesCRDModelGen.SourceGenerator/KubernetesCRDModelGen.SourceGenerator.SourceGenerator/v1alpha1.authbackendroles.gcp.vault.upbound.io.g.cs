...
/// <summary>AuthBackendRole is the Schema for the AuthBackendRoles API. Managing roles in an GCP auth backend in Vault</summary>
...
{
    [JsonPropertyName("addGroupAliases")]
    ...
    /// <summary>A flag to determine if this role should allow GCE instances to authenticate by inferring service accounts from the GCE identity metadata token.</summary>
    [JsonPropertyName("allowGceInference")]
    public bool? AllowGceInference { get; set; }

    /// <summary>Path to the mounted GCP auth backend</summary>
    ...
    /// <summary>The instance groups that an authorized instance must belong to in order to be authenticated. If specified, either bound_zones or bound_regions must be set too.</summary>
    [JsonPropertyName("boundInstanceGroups")]
    public IList<string>? BoundInstanceGroups { get; set; }

    /// <summary>A comma-separated list of GCP labels formatted as &quot;key:value&quot; strings that must be set on authorized GCE instances. Because GCP labels are not currently ACL&apos;d, we recommend that this be used in conjunction with other restrictions.</summary>
    [JsonPropertyName("boundLabels")]
    public IList<string>? BoundLabels { get; set; }

    /// <summary>An array of GCP project IDs. Only entities belonging to this project can authenticate under the role.</summary>
    [JsonPropertyName("boundProjects")]
    public IList<string>? BoundProjects { get; set; }

    /// <summary>The list of regions that a GCE instance must belong to in order to be authenticated. If bound_instance_groups is provided, it is assumed to be a regional group and the group must belong to this region. If bound_zones are provided, this attribute is ignored.</summary>
    [JsonPropertyName("boundRegions")]
    public IList<string>? BoundRegions { get; set; }

    /// <summary>GCP Service Accounts allowed to issue tokens under this role. (Note: Required if role is iam)</summary>
    [JsonPropertyName("boundServiceAccounts")]
    public IList<string>? BoundServiceAccounts { get; set; }

    /// <summary>The list of zones that a GCE instance must belong to in order to be authenticated. If bound_instance_groups is provided, it is assumed to be a zonal group and the group must belong to this zone.</summary>
    [JsonPropertyName("boundZones")]
    public IList<string>? BoundZones { get; set; }

    /// <summary>The number of seconds past the time of authentication that the login param JWT must expire within. For example, if a user attempts to login with a token that expires within an hour and this is set to 15 minutes, Vault will return an error prompting the user to create a new signed JWT with a shorter exp. The GCE metadata tokens currently do not allow the exp claim to be customized.</summary>
    [JsonPropertyName("maxJwtExp")]
    ...
    /// <summary>Name of the GCP role</summary>
    ...
    /// <summary>Type of GCP authentication role (either gce or iam)</summary>
